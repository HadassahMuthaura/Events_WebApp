...
const cancelBookingRecord = async (booking) => {
...
export const createBooking = async (req, res) => {
  try {
    ...
    // Create booking with generated reference number
    const booking_reference = generateReferenceNumber();
...
export const cancelBooking = async (req, res) => {
  try {
    ...
    // Check if user owns this booking or elevated role
    if (booking.user_id !== req.user.id && req.user.role !== 'admin' && req.user.role !== 'superadmin') {
      return res.status(403).json({ error: 'Access denied' });
    }
...
export const getBookingByReference = async (req, res) => {
  try {
    ...
    const { data: booking, error } = await supabase
      ...
      .select(`
        *,
        events:events!bookings_event_id_fkey(id, title, date, location, category, image_url, venue),
        users:users!bookings_user_id_fkey(id, full_name, email)
      `)
    ...
    const canSupportLookup = role === 'support' || role === 'admin' || role === 'superadmin';

    if (!isOwner && !canSupportLookup) {
      return res.status(403).json({ error: 'Access denied' });
    }
...
export const cancelBookingByReference = async (req, res) => {
  try {
    ...
    res.json({ message: 'Booking cancelled successfully', booking_reference: reference });
  } catch (error) {
    console.error('Cancel booking by reference error:', error);
    res.status(500).json({ error: 'Failed to cancel booking' });
  }
};
