...
// Show Create Event only for admins and superadmins
  const canCreateEvent = user?.role === 'admin' || user?.role === 'superadmin'
  const canUseScanner = user?.role === 'organizer' || user?.role === 'admin' || user?.role === 'superadmin'
  const canUseInsights = user?.role === 'organizer' || user?.role === 'admin' || user?.role === 'superadmin'
  ...
  const menuItems = [
    ...
    ...(canUseScanner ? [{ icon: FiCamera, label: 'Ticket Scanner', href: '/dashboard/scanner' }] : []),
    { icon: FiFileText, label: 'Event Details', href: '/dashboard/event-details' },
    ...(canUseInsights ? [{ icon: FiUsers, label: 'Attendee Insights', href: '/dashboard/attendee-insights' }] : []),
  ...
  return (
    <>
      ...
      <div className={`
        ...
        fixed md:relative left-0 top-0 md:left-auto md:top-auto shadow-2xl md:shadow-none
